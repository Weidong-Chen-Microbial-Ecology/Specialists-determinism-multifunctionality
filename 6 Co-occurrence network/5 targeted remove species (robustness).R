...
##read otu table
otutab<-read.csv("OTU.csv",header = T,row.names=1)
...
write.table(cormatrix,'cormatrix.txt')
...
cormatrix.input[row(cormatrix.input) >= col(cormatrix.input)] <- scan("CorrelationMatrix_Y14_W.txt")
...
cormatrix2<-cormatrix*(abs(cormatrix)>=0.60)  #only keep links above the cutoff point
...
#1 remove keystone species
#get the keystone species list
node.attri<-read.csv("zipi.txt",header = T, sep="\t")
module.hub<-as.character(node.attri$Name[node.attri$Zi > 2.5 & node.attri$Pi <= 0.62])

#2 remove opportunists、generalists and specialists
module.hub <- as.character(node.attri$Name[node.attri$sign == "NON SIGNIFICANT"])
#module.hub <- as.character(node.attri$Name[node.attri$sign == "GENERALIST"])
#module.hub <- as.character(node.attri$Name[node.attri$sign == "SPECIALIST"])
...
currentdat = dat1
write.csv(Weighted.simu,"Result.csv")
